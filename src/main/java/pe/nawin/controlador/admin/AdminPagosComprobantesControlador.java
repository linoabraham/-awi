...
@Tag(name = "15 ADMIN - Pagos y comprobantes", description = "Pagos, series, comprobantes, anulaciones y PDF privado.")
public class AdminPagosComprobantesControlador {

	private final PagoServicio pagoServicio;
	private final ComprobanteServicio comprobanteServicio;

	public AdminPagosComprobantesControlador(PagoServicio pagoServicio, ComprobanteServicio comprobanteServicio) {
	...
	public ResponseEntity<RespuestaApi<Map<String, Object>>> crearSerie(@Valid @RequestBody SerieComprobanteRequest request) {
		return ResponseEntity.ok(RespuestaApi.ok(comprobanteServicio.crearSerie(request)));
	}

	@GetMapping("/series-comprobantes")
	public ResponseEntity<RespuestaApi<List<Map<String, Object>>>> listarSeries(@RequestParam(required = false) Boolean activo) {
		return ResponseEntity.ok(RespuestaApi.ok(comprobanteServicio.listarSeries(activo)));
	}

	@GetMapping("/series-comprobantes/{id}")
	public ResponseEntity<RespuestaApi<Map<String, Object>>> verSerie(@PathVariable Long id) {
		return ResponseEntity.ok(RespuestaApi.ok(comprobanteServicio.verSerie(id)));
	}

	@PutMapping("/series-comprobantes/{id}")
	public ResponseEntity<RespuestaApi<Map<String, Object>>> actualizarSerie(@PathVariable Long id,
			@Valid @RequestBody SerieComprobanteRequest request) {
		return ResponseEntity.ok(RespuestaApi.ok(comprobanteServicio.actualizarSerie(id, request)));
	}

	@DeleteMapping("/series-comprobantes/{id}")
	public ResponseEntity<RespuestaApi<Map<String, Object>>> inactivarSerie(@PathVariable Long id) {
		comprobanteServicio.inactivarSerie(id);
		return ResponseEntity.ok(RespuestaApi.ok(Map.of()));
	}
	...
	public ResponseEntity<RespuestaApi<Map<String, Object>>> noModificar(@PathVariable Long id) {
		return ResponseEntity.ok(RespuestaApi.ok(comprobanteServicio.ver(id)));
	}
